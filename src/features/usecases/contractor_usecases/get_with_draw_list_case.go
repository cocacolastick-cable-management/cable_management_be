...
func (gwd GetWithDrawListCase) Handle(accessToken string) ([]*responses.WithDrawResponse, error) {
	...
	claims, err := gwd.makeSureAuthorized.Handle(accessToken, constants.ContractorRole)
	if err != nil {
		return nil, err
	}

	withDrawList, _ := gwd.withDrawRepo.FindManyByContractorId(claims.AccountId, []string{"Histories", "Histories.Creator", "Contract", "Contract.Supplier", "Contractor"})
